...
### Minimax (`src/minimax/`)
Negamax with alpha-beta, a transposition table and killer moves, over a `SearchState` that knows, for every empty cell and colour, what one more stone there would make. The evaluation is the classical one of engines such as Rapfi: a position is worth the sum of its cells' threats.

- **`lines.rs`**: line geometry (`LINE_LENGTHS`, `POSITION_LINES`, `LINE_CELLS`, `NEIGHBOURHOODS`) and the cell functions on a `u16` line mask: which empty cells complete five, make a four, a three, a two, an open four, an open three, an open two, and which cells stop a four or an open four in the making (`four_breaking_cells`, `defusing_cells`). Tests check every cell function against a brute-force reading of every colouring of a 9-cell line.
- **`patterns.rs`**: `LinePattern` is what a stone makes on one line (`Nothing` … `Five`), read by table from the eight cells within `REACH` of the cell (`line_pattern`; the board edge reads as blocked). `CellPatterns` packs a cell's four line patterns; `Threat` is their combination for the cell, in Rapfi's priority order (`OpenTwo` … `UnstoppableFour`, `Five`), also by table, with a value per class in `Threat::value`. A cell where two fours or an open four can be made is `UnstoppableFour`.
- **`SearchState`**: stones as one `u16` mask per colour per line, plus every empty cell's `CellPatterns` for both colours, the cells grouped by `Threat` in one `BitBoard` per class, and each colour's threat total. A stone only changes the cells within `REACH` on its four lines, and each of those shares exactly one line with it, so `place` rewrites one nibble per cell and `undo` restores the saved patterns. `evaluate` is the raw difference of the two totals for the side to move. A quiet leaf is scored as the average of its own raw evaluation and the parent position's, seen from the same side (`Search::leaf_value`, Rapfi's `(eval - rawStaticEval[ply - 1]) / 2`): the newest stone's threats are unanswered, so whoever moved last always looks ahead by thousands of points, and averaging over the last move takes half of that swing out. Without it a depth-1 Black beat a depth-6 White 20 games to 0; weighting the side to move instead (1.5 or 2) only moved the failures to other depth pairings. `Five` and `UnstoppableFour` cells are worth nothing, since the search always plays them out and a value would leak in through the parent of a leaf reached by a forced block. Leaves depend on their parent, so only nodes with depth left use the transposition table.
- **`Situation`**: what the side to move faces, most urgent first: `CompleteFive`, `BlockFour`, `MakeUnstoppableFour` (a proven win in three), `PreventUnstoppableFour`, `Develop`. `candidates` derives the moves: forced replies first (the cells completing a four; the threat cell itself, anything along an open three's line that stops it, or the window a four would fill), then chosen moves (counter-fours, or every empty cell within two of a stone ordered by cell value). A test checks over random positions that the forced replies include every move that removes the threat.
- **Depth**: every move costs one, but a forced reply is searched even at depth 0, so a position with a four or an unstoppable four in the making is never scored statically. Forced replies must not be free: the evaluation credits whoever moved last, so lines ending one stone later would make threats look good for their own sake (depth 4 lost to depth 2 that way). Counter-fours are ordinary chosen moves for the same reason and to keep the tree bounded; a test holds a dense middle-game position to a node budget.
- Depth 0 is rejected everywhere (`minimax:0`, `--minimax-scoring-depth 0`). `find_best_move` logs the node count at debug level.
- **Cost**: `place` and `undo` are the whole profile (each touches up to 32 cells for both colours). The two 64 KB lookup tables are `LazyLock`s; `place` fetches each once and passes the reference down (`threat_from`, `line_pattern_from`), and every cell's current `Threat` is cached so a change costs one lookup, not two. Together that was a third of the per-node instructions. Compare builds by instructions per node on `play minimax:2 minimax:5 --seed 42` (nodes from `-l debug`), since the evaluation decides the moves and wall time alone is not comparable across evaluation changes.
