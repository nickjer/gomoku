...
- **`SearchState`**: stones as one `u16` mask per colour per line, plus every empty cell's `CellPatterns` for both colours, the cells grouped by `Threat` in one `BitBoard` per class, and each colour's threat total. A stone only changes the cells within `REACH` on its four lines, and each of those shares exactly one line with it, so `place` rewrites one nibble per cell and `undo` restores the saved patterns. `evaluate` is the totals' difference plus the side to move's best cell once more (it gets to play it).
