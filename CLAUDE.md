...
- **Scored boards** (`fitness_evaluator/scored_board.rs`): `data/scored_boards.jsonl` holds boards from Gomocup 2025 games and our minimax self-play with minimax's depth-4 score for every empty position. One JSON object per line: `stones`, 225 characters of `.`/`X`/`O`; `scores`, 225 entries from the side to move's view, `null` under a stone, a bare integer for an estimate in points, `{"win": n}` or `{"loss": n}` when the search saw the game end after `n` more stones (always odd for a win, even for a loss); and `depth`, the search depth. The file never holds a raw win or loss score: the generator (`tmp/positions/tools/compact_scored_boards.py`, outside the repository) undoes the search's `WIN - move_count` encoding, and the reader rejects any estimate of ten million points or more. At load an estimate is its points and a game end is worth the file's largest estimate (977 on the depth-4 file, logged at load) times `1 + nearness_weight * 2 / n` (`--nearness-weight`, default 2: a five after two stones weighs 3 largest estimates, after four 2, after ten 1.4; 0 weighs every game end one), so every win ranks above every quiet move and every loss below whatever scale a rescored file has. The largest estimate is an observation, not a bound: the evaluation sums a table value of up to 378 per cell, and the depth-4 file's 99.99th percentile is 803. An earlier version clamped estimates to a fixed cap of 1000 and weighed every game end one cap; measured then on the depth-4 file, leaving a four unblocked cost the same shortfall as a loss minimax found ten stones deep, and on the median board without a four 83% of the empty cells were such deep losses; forced-block boards were a quarter of a cluster-small model's remaining error and it blocked only 20% of them after 9,600 generations. Values are turned into a **shortfall**, how far a position falls behind the board's best as a share of the largest estimate; boards where every move ties are dropped. Each generation samples `--boards-per-generation` boards once, so every strategy is measured on the same boards. A board's error is the shortfall of each ranked position weighted by `rank_decay^(k-1)` (`--rank-decay`, default 0.1; 0 counts only the pick), normalized; fitness is minus the mean error. Ordering only, no raw outputs: measured networks span 17 orders of magnitude with heavy tails, so softmax on outputs is a staircase. Logs per generation the mean pick shortfall in points, the blunder rate (pick shortfall at least 0.9), and the share of picks tying the best. The old `--minimax-scoring-depth` mode compares searches with different horizons and its length term dominates; it stays as it is.
...
- **Fail-soft**: `negamax` returns and stores the best value a searched line produced, never the window's edge, and a table cutoff returns the stored bound. A win or loss counts the stones on the board at the five, so an edge one point off such a score is a five one stone away from where it can fall; with fail-hard the one-point null windows put such edges in the table and three of the 4.1 million cells in `data/scored_boards.jsonl` came out with a win on an even stone or a loss on an odd one (the parity test in `search.rs` holds one of them). Measured against fail-hard on 240 seeded games at depths 2 to 5 with 4 opening stones: every game identical move for move, 8% to 15% fewer nodes per move.
...
`inspect --board FILE` also scores the board drawn in `FILE` (rows of `X`, `O` and `.` or `·`, whitespace ignored) for the side to move and prints every empty position's rank and the ten favorites with their scores. Useful for asking why a strategy ignored a threat: a strategy's score at a position depends only on the stones within as many cells as it has 3x3 layers, so a two-layer kind cannot tell the end of a four from the end of a two.
...
- `--nearness-weight` [2]: Extra multiples of the file's largest estimate a win or loss two stones away is worth, halving with distance (0 = every win or loss is worth the largest estimate)
