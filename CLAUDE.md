...
The release build includes debug symbols for profiling. Use `perf` for CPU profiling:
...
Avoid using `--call-graph dwarf` with `perf record` as it generates large files and `addr2line` can hang during analysis.
