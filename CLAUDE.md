...
- **`lines.rs`**: line geometry (`LINE_LENGTHS`, `POSITION_LINES`, `LINE_CELLS`, `NEIGHBOURHOODS`) and, for tests only, the cell functions on a `u16` line mask (which empty cells complete five, make a four, a three, an open four, …), each checked against a brute-force reading of every colouring of a 9-cell line and then used to check the ported classifier.
