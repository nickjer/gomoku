use crate::cluster::fingerprint::{Fingerprint, FingerprintNN3};
...
/// Cache of precomputed fingerprint indices for NN3 fingerprints.
#[derive(Debug, Clone)]
pub struct FingerprintNN3IndexCache {
...
impl FingerprintNN3IndexCache {
    ...
    pub fn new() -> Self {
        ...
        let black_indices = PositionMap::from_fn(|pos| {
            ...
            FingerprintNN3::new(nn1, nn2, nn3).index()
        });
...
impl Default for FingerprintNN3IndexCache {
    fn default() -> Self {
        Self::new()
    }
}

impl super::FingerprintIndexCache for FingerprintNN3IndexCache {
...
mod tests {
    ...
    fn initial_indices_match_empty_board() {
        let cache = FingerprintNN3IndexCache::new();
        let center = PositionId::center();

        let expected = FingerprintNN3::new(
