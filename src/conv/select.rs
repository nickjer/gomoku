...
pub fn select_best_position(
    ...
    let mut best_value = policy[first];
    let mut tie_count = 1;

    for &pos in rest {
        let value = policy[pos];
...
mod tests {
    ...
    fn policy_with_values(values: &[(PositionId, f32)]) -> Vec<f32> {
        let mut data = vec![f32::NEG_INFINITY; PositionId::COUNT];
        for &(pos, value) in values {
            data[usize::from(pos)] = value;
        }
        data
    }
    ...
    fn selects_highest_value() {
        ...
        let policy = PositionSlice::new(&policy_data);
